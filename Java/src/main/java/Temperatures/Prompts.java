...
public class Prompts {
    ...
    public static String calculationPrompt(String startingScale, String endingScale, double startingTempCalc, String startingTemp) {
        ...
        String comparingScales = startingScale + " & " + endingScale;
        double result;

        switch (comparingScales) {
            case "F & C":
                result = Calculations.fahrenheitToCelsius(startingTempCalc);
                System.out.println(startingTemp + "°F is " + result + "°C");
                break;
            case "C & F":
                result = Calculations.celsiusToFahrenheit(startingTempCalc);
                System.out.println(startingTemp + "°C is " + result + "°F");
                break;
            case "K & C":
                result = Calculations.kelvinToCelsius(startingTempCalc);
                System.out.println(startingTemp + "°K is " + result + "°C");
                break;
            case "F & K":
                result = Calculations.fahrenheitToKelvin(startingTempCalc);
                System.out.println(startingTemp + "°F is " + result + "°K");
                break;
            case "C & K":
                result = Calculations.celsiusToKelvin(startingTempCalc);
                System.out.println(startingTemp + "°C is " + result + "°K");
                break;
            case "K & F":
                result = Calculations.kelvinToFahrenheit(startingTempCalc);
                System.out.println(startingTemp + "°K is " + result + "°F");
                break;
            default:
                System.out.println("Unsupported input. Please try again.");
                System.out.println("");
                Prompts.endingScalePrompt(startingScale);
        }
